...
TODO_FILE = "todo.json"
# JSON 파일 잠금 경로
TODO_LOCK = f"{TODO_FILE}.lock"

# JSON 파일에서 To-Do 항목 로드
def load_todos():
    with FileLock(TODO_LOCK):
...
def update_todo(todo_id: str, updated_todo: TodoItem):
    ...
    for todo in todos:
        if str(todo["id"]) == str(todo_id):
            ...
            data["id"] = str(todo_id)  # Ensure ID remains unchanged
            todo.update(data)
            save_todos(todos)
            return todo
    raise HTTPException(status_code=404, detail="To-Do item not found")

# To-Do 항목 상태 토글 (완료/미완료)
...
def update_priority(todo_id: str, priority_update: PriorityUpdate):
    todos = load_todos()
    for todo in todos:
        if str(todo["id"]) == str(todo_id):
            todo["priority"] = priority_update.priority
            save_todos(todos)
            return todo
    raise HTTPException(status_code=404, detail="To-Do item not found")
